...
void find_road()
{
    ...
    for (int i = 0; i < 120; i++)
    {
        for (int k = 0; k <= my_road[i].white_num; k++)
        {
            for (int j = my_road[i].connected[k].left; j <= my_road[i].connected[k].right; j++)
            {
                IMG[i][j] = gray;
            }
        }
...
void fix_un()
{
    uint8_t begin_left = NEAR_LINE;
    uint8_t begin_right = NEAR_LINE;
    for (int i = 100; i >= 0; i--) //119-102为清车头范围
    {
        if (IsCross(i,left_line[i]-1,3))
        {
            begin_left = i + 2;
            break;
        }
    ...
    if (begin_left != NEAR_LINE && begin_right != NEAR_LINE)
    {
        ...
        k_left = K_line(NEAR_LINE, begin_left, 0);
        k_right = K_line(NEAR_LINE, begin_right, 1);
        for (int i = begin_left; i >= 0; i--)
        {
            left_line[i] = left_line[begin_left] + (uint8_t)(k_left * (float)(i - begin_left));
            if ((i < begin_left - 4) && (IMG[i][left_line[i]] != white))
            {
                break;
            }
        ...
        {
            right_line[i] = right_line[begin_right] + (uint8_t)(k_right * (float)(i - begin_right));
            if ((i < begin_right - 4) && (IMG[i][right_line[i]] != white))
            {
                break;
            }
...
float K_line(uint8_t Begin, uint8_t Final, uint8_t sign)
{
    float k = 0;
...
uint8_t image_main(uint8_t midline)
{
    ...
    //}
    return mid_line[midline];
}

//粗略判断是否是十字交叉点
//行数为x,列数是y,Num为用于判断的行数(每行一个点)
int IsCross(int x, int y, int Num)
{
    int count = 0;
    for (int i = 0; i < Num; i++)
    {
